...
def growDecisionTreeFrom(rows, evaluationFunction=entropy):
	...
	bestAttribute = None
	bestSets = None
	# last column is the result/target column
...
if __name__ == '__main__':
	...
	if example == 1:
		...
		trainingData= pandas.read_csv("testing", header=None, names = col_names)
		...
		# the bigger example
		# trainingData = loadCSV('testing.') # demo data from matlab
		trainingData= pandas.read_csv("testing", header=None, names = col_names)
		decisionTree = growDecisionTreeFrom(trainingData)		
		plot(decisionTree)
